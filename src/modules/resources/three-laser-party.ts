...
const laserMain = {
  ...
  transforms: {
    init(data) {
      ...
      this.angleX = this.defaults.angleX = laserMain.getValueOrDefault(
        "angleX",
        data
      );

      this.angleY = this.defaults.angleY = laserMain.getValueOrDefault(
        "angleY",
        data
      );

      this.spin = this.defaults.spin = laserMain.getValueOrDefault(
        "spin",
        data
      );
    },
  ...
  init() {
    ...
    this.planeGeometry = new PlaneGeometry(1, 0.1);

    this.baseBoxGeometry = new BoxGeometry(0.4, 0.25, 0.5);
    this.baseBoxMaterial = new MeshPhongMaterial({
      color: 0x111111,
      side: FrontSide,
    });

    this.baseBox = new Mesh(this.baseBoxGeometry, this.baseBoxMaterial);
  ...
  createGradientCanvases() {
    ...
    var g = c.createLinearGradient(0, 0, c.canvas.width, c.canvas.height);
      ...
      if (typeof this.animation == "string") {
        if (!animMain.all[this.animation]) {
          laserMain.doUniqueError(
            `Laser Animation: Movement animation not found: ` + this.animation
          );
        } else {
          this.animation = animMain.all[this.animation];
        }
      }

      if (this.animation.isLaserAnim)
        laserMain.validateAnimationType("movement", this.animation);

      animMain.run.call(this.animation, time, deltaTime, this);
    }
    ...
    if (this.colorAnimation) {
      if (typeof this.colorAnimation == "string") {
        if (!animMain.all[this.colorAnimation]) {
          laserMain.doUniqueError(
            `Laser Animation: Color animation not found: ` + this.colorAnimation
          );
        } else {
          this.colorAnimation = animMain.all[this.colorAnimation];
        }
      }

      if (this.colorAnimation.isLaserAnim)
        laserMain.validateAnimationType("color", this.colorAnimation);

      animMain.run.call(this.colorAnimation, time, deltaTime, this);
    }
    ...
    if (type != animation.type) {
      laserMain.doUniqueError(
        `Laser Animation: Invalid animation type` +
          ...
          (animation.name ? `Name: ` + animation.name : "unnamed") +
          `. Type: ${animation.type}`
      );
    }
  },
  ...
  getValueOrDefault(property, data) {
    ...
    else if (data[property] != undefined) return data[property];
  ...
  clearDisposed() {
    ...
    if (index == -1) return console.error("This shouldnt happen lol");
    ...
    for (let id of this.groups) {
      ...
      if (objIndex == -1) return console.error("This shouldnt happen lol");
  ...
  setBeamProperties(beamParent) {
    ...
    Object.defineProperty(beamParent, "hue", {
      ...
      set: function (value) {
        ...
        if (_hue < 0)
          _hue = Math.round(_hue + Math.ceil(Math.abs(_hue / 360)) * 360);
    ...
    Object.defineProperty(beamParent, "convertedSaturation", {
      get: function () {
        return Math.floor(_saturation * 100);
      },
    });

    Object.defineProperty(beamParent, "convertedLightness", {
      get: function () {
        return Math.floor(_lightness * 100);
      },
    });
  },
  setBeamParentHue() {
  ...
  createBeam(data = {}) {
    ...
    const spriteScale = 0.1 * this.thickness;
  ...
  createBaseBox(data) {
    if (!laserMain.getValueOrDefault("baseBox", data)) return;

    this.baseBox = laserMain.baseBox.clone();
    this.add(this.baseBox);

    this.baseBox.position.y = 0.125;

    // adjust plane start point to box
    this.laserWrap.position.set(0, 0.125, 0.25);
...

    {
      ...
      this.baseSprite.scale.set(spriteScale, spriteScale, spriteScale);
    }

    laserMain.setBaseSpriteLightness.call(this);
    this.baseSprite.position.z = 0.01;
  ...
  removeLastBeam() {
    ...
    beam.traverse((item) => tempToClear.push(item));

    for (let i = tempToClear.length - 1; i >= 0; i--) {
      tempToClear[i].clear();

      tempToClear.material && tempToClear.material.dispose();
    }
  ...
  initGettersSetters(data) {
    // HSL
    let _hue = (this.defaults.hue = laserMain.getValueOrDefault("hue", data));
    ...
    Object.defineProperty(this, "hue", {
      ...
      set: function (value) {
        ...
        for (let beamParent of this.allBeamParents) {
          laserMain.setBeamParentToLaserHSL.call(this, beamParent);
        }
      },
    });

    let _saturation = (this.defaults.saturation = laserMain.getValueOrDefault(
      "saturation",
      data
    ));
    ...
    let _lightness = (this.defaults.lightness = laserMain.getValueOrDefault(
      "lightness",
      data
    ));
    ...
    // BEAM
    let _distance = laserMain.getValueOrDefault("distance", data);
    ...
    Object.defineProperty(this, "distance", {
      ...
      set: function (value) {
        ...
        this.raycast.enabled && this.raycast.once();
      },
    });

    let _thickness = (this.defaults.thickness = laserMain.getValueOrDefault(
      "thickness",
      data
    ));
    ...
    Object.defineProperty(this, "thickness", {
      ...
      set: function (value) {
        const baseSpriteValue = value * 0.1;
        this.baseSprite &&
        ...
        _thickness = value;
        const spriteValue = value * 0.1;

        for (let [i, planeParent] of Object.entries(this.allPlaneParents)) {
          this.allPointSprites[i].scale.set(
            spriteValue,
            spriteValue,
            spriteValue
          );
    ...
    let _spread = (this.defaults.spread = laserMain.getValueOrDefault(
      "spread",
      data
    ));
    ...
    let _side = (this.defaults.side = laserMain.getValueOrDefault(
      "side",
      data
    ));
    ...
    // ANIMATION
    let _speed = (this.defaults.speed = laserMain.getValueOrDefault(
      "speed",
      data
    ));
    ...
    let _animation = (this.defaults.animation = laserMain.getValueOrDefault(
      "animation",
      data
    ));
    ...
    let _colorAnimation = (this.defaults.colorAnimation =
...
class LaserObject extends Object3D {
  ...
  updateSpreadAndSide = (function iIFE() {
    ...
    return function updateSpreadAndSideInner() {
      if (this.side === _lastSide && this.spread === _lastSpread) return;

      _lastSide = this.side;
      _lastSpread = this.spread;

      var len = this.allBeamParents.length;
      var offset = 0.5 / len; // used to center all of the planes
      ...
      for (const [i, beamParent] of this.getBeamParents) {
        ...
        const rawValue =
          ...
          ((t + offset) * Math.PI +
            ...
            this.spread +
          // side value
          Math.PI * this.side * (0.47 + this.spread / 2);
  ...
  reset(zero) {
    if (zero === 0) {
      ...
      this.defaults.pointAt && this.pointAt(this.defaults.pointAt);
    }
    this.update();
  }
  resetColor(zero) {
  ...
  pointAt(objOrVec3) {
    this.laserWrap.lookAt(objOrVec3.isVector3 ? objOrVec3 : objOrVec3.position);
  }
  addTo(what) {
    what.add(this);
  }
  ...
  apply(pose) {
    if (!pose.isLaserPose)
      return console.error(`Laser Apply: Invalid pose: ${pose}`);
  ...
  // viibility things
  get enabled() {
    return this.laserWrap.visible;
  }
  get convertedLightness() {
    return Math.floor(this.lightness * 100);
  }
  get convertedSaturation() {
    return Math.floor(this.saturation * 100);
  }
  // laserWrap rotation getters
  get angleX() {
    return this.laserWrap.rotation.x * -1;
  }
  get angleY() {
    return this.laserWrap.rotation.y;
  }
  get spin() {
    return this.laserWrap.rotation.z;
  }
  ...
  // beam count
  get count() {
    return this.allBeamParents.length;
  }
  set count(value) {
...
const groupMain = {
  ...
  validateLaser(laserObj) {
    if (!laserObj.isLaser)
      console.error(`Laser Group: Can't add a non-laser: ${laserObj}`);
    else return true;
  },
  parseInitData(data) {
    const laserArray = [];
    ...
    for (let objOrAry of data) {
      ...
      if (Array.isArray(objOrAry)) {
        for (let laser of objOrAry) {
          ...
          if (groupMain.validateLaser(laser)) laser.groups.push(this.id);

          // add to laserArray if unique
          if (!laserArray.includes(objOrAry)) laserArray.push(laser);
        }
      }
      ...
      // single laser
        ...
        // add group id to laser
        if (!objOrAry.groups.includes(this.id)) objOrAry.groups.push(this.id);

        // add to laserArray if unique
        if (!laserArray.includes(objOrAry)) laserArray.push(objOrAry);
      }
    }
  ...
  validateAndAdd(laserObj) {
    if (groupMain.validateLaser(laserObj) && !this.lasers.includes(laserObj)) {
...
const poseMain = {
  ...
  validatePose(pose) {
    ...
    else console.error(`Laser Pose: Invalid pose property: ${pose}`);
  },
  validateAndParseData(data) {
    this.pose = {};

    for (let [key, value] of Object.entries(data)) {
      if (poseMain.extraProperties.includes(key)) this[key] = value;
      else if (poseMain.validatePose(key)) this.pose[key] = value;
    }
  },
  applyPose(laser) {
  ...
  checkGroupOrLaserThenApply(laserOrGroup) {
    if (laserOrGroup.isLaser) poseMain.applyPose.call(this, laserOrGroup);
    else if (laserOrGroup.isLaserGroup)
      poseMain.applyGroupPose.call(this, laserOrGroup);
  },
  applyGroupPose(group) {
    for (let laser of group.lasers) poseMain.applyPose.call(this, laser);
  },
};
const poseObject = {
  init(data = {}) {
    ...
    for (let laserOrGroup of laserOrGroups) {
      if (Array.isArray(laserOrGroup)) {
        for (let nestedLaserOrGroup of laserOrGroup) {
          poseMain.checkGroupOrLaserThenApply.call(this, nestedLaserOrGroup);
        }
      } else {
        poseMain.checkGroupOrLaserThenApply.call(this, laserOrGroup);
      }
    }
  },
  get entries() {
    return Object.entries(this.pose);
  },
};
...
const presetMain = {
  validOptions: ["pose", "target", "animation", "colorAnimation"],
  dataIsInvalid(data) {
    ...
    for (let setting of data) {
      ...
      if (!setting.target) {
        console.error(`Laser Preset: Missing required target`);
        failedTest = true;
      }

      // validate target type
      else if (!setting.target.isLaser && !setting.target.isLaserGroup) {
        console.error(
          `Laser Preset: Target must be ` + `a laser or a laser group.`
        );
        failedTest = true;
      }
      ...
      if (setting.pose && !setting.pose.isLaserPose) {
        console.error(`Laser Preset: Invalid pose:`, setting.pose);
        failedTest = true;
      }
    }
  ...
  parseAndValidateData(data) {
    ...
    for (let setting of data) {
      ...
      for (let [key, value] of Object.entries(setting)) {
        if (presetMain.validateOption(key)) parsedSetting[key] = value;
      }
  ...
  validateOption(option) {
    if (presetMain.validOptions.includes(option)) return true;
    else console.error(`Laser Preset: Invalid option: ${option}`);
  },
  doApply(setting) {
    // apply pose
  ...
  dispatchApply(presetSetting) {
    // apply to lasers
    if (presetSetting.target.isLaser) presetMain.doApply(presetSetting);
...
const presetObject = {
  init(...data) {
    ...
    presetMain.parseAndValidateData.call(this, data);
...
const animMain = {
  all: {},
  validOptions: ["name", "type", "fn"],

  // init functions
  typeIsInvalid({ type }) {
    if (type == "color" || type == "movement") return false;

    console.error(`Laser Animation: Type must be 'color' or 'movement'`);
    return true;
  },
  nameIsInvalid({ name }) {
    ...
    if (typeof name != "string") {
      console.error(`Laser Animation: Name must be a string: ${name}`);
      return true;
    }
    ...
    if (found) {
      console.error(`Laser Animation: Name already exists: ${name}`);
      return true;
    }
  ...
  fnIsInvalid({ fn }) {
    if (typeof fn != "function") {
      console.error(`Laser Animation: fn: type must be function`);
      return true;
    } else return false;
  },
  dataIsInvalid(data) {
  ...
  parseData(data) {
    ...
    for (let [key, value] of Object.entries(data)) {
      if (animMain.validOptions.includes(key)) {
        this[key] = value;
      } else console.error(`Laser Animation: Invalid option: ${key}`);
    }
  },
...
const animObject = {
  init(data = {}) {
    ...
    if (this.name != undefined) animMain.all[this.name] = this;
...
const raycast = {
  nonObj3DErr(obj) {
    console.error(`Laser Raycast: Can't add a non-Object3D: `, obj);
  },
  // LOCAL
    ...
    for (let obj3dOrArray of array) {
      if (Array.isArray(obj3dOrArray)) {
        for (let obj3d of obj3dOrArray) {
          raycast.localValidateAndAddRaycastObject.call(this, obj3d);
        }
      } else {
        raycast.localValidateAndAddRaycastObject.call(this, obj3dOrArray);
      }
    }
  },
  remove(...array) {
    for (let obj3dOrArray of array) {
      if (Array.isArray(obj3dOrArray)) {
        for (let obj3d of obj3dOrArray) {
          raycast.localRemoveRaycastObject.call(this, obj3d);
        }
      } else {
        raycast.localRemoveRaycastObject.call(this, obj3dOrArray);
      }
    }
  },
  addGlobal(...array) {
  ...
  scaleBeamToFullDistance(i) {
    this.allPlaneParents[i].scale.x = this.distance / this.scale.x;
  ...
  offsetBeamAfterScaling(i) {
    ...
    this.allPlaneParents[i].position.z = this.allPlaneParents[i].scale.x / 2;
  },
  doRaycast() {
    ...
    for (var i = 0; i < len; i++) {
      ...
      laserMain.raycaster.ray.direction
        .set(0, 0, 1)
        .applyMatrix4(matrixClone)
        .normalize();

      // check intersections
      var intersects = laserMain.raycaster.intersectObjects([
      ...
      if (intersects.length) {
        this.allPlaneParents[i].scale.x =
          MathUtils.clamp(intersects[0].distance, 0, this.distance) /
          this.scale.x;

        this.allPointSprites[i].position.z =
          this.allPlaneParents[i].scale.x - 0.01;
...
const laserParty = {
  new(data) {
    return new LaserObject(data);
  },
  pose(data) {
    return Object.create(poseObject).init(data);
  },
  anim(data) {
    return Object.create(animObject).init(data);
  },
  group(...data) {
    return Object.create(groupObject).init(...data);
  },
  preset(...data) {
    return Object.create(presetObject).init(...data);
  },
  ...
  get globalAnimationSpeed() {
    return laserMain.globalAnimationSpeed;
  },
  set globalAnimationSpeed(speed) {
    laserMain.globalAnimationSpeed = speed;
  },
      ...
      for (let [key, value] of Object.entries(obj)) {
        ...
        // group log the valid options
          ...
          let validOptions = Object.keys(laserMain.defaults).reduce(
            (a, c) => `${a}   ${c} ( ${typeof laserMain.defaults[c]} )\n`,
            "\n"
          );
...
for (let key in laserMain.defaults) {
  if (key == "addTo") {
    Object.defineProperty(laserParty.defaults, key, {
      get: function () {
        return laserMain.defaults.addTo;
      },
      set: function (value) {
...
export const Laser = LaserObject;

{
  ...
  laserParty.anim({
    ...
    type: "movement",
    fn({ time }) {
      this.angleX = Math.sin(time) * 0.2;
      this.angleY = Math.cos(time) * 0.2;
    },
  });
  // Color
  laserParty.anim({
    ...
    type: "color",
    fn({ time }) {
      this.hue = (Math.sin(time) * 0.5 + 0.5) * 180;
    },
  });
  // Hue
  laserParty.anim({
  ...
  laserParty.anim({
    ...
    type: "color",
    fn({ time }) {
      this.saturation = Math.sin(time) * 0.5 + 0.5;
    },
  });
  // Lightness
  laserParty.anim({
    ...
    type: "color",
    fn({ time }) {
      this.lightness = Math.sin(time) * 0.5 + 0.5;
    },
  });
  // Side
  laserParty.anim({
